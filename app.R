...
ui <- page_navbar(
  ...
  header = tags$head(
    ...
    tags$audio(id = "bgm", src = "bgm.mp3", type = "audio/mp3", preload = "auto", style = "display:none;"),
    tags$audio(id = "success_sound", src = "success.mp3", type = "audio/mp3", preload = "auto", style = "display:none;"),
       ...
       function startBGM(){
         ...
         if(a){
           ...
           if(p && typeof p.then==='function'){
             p.then(function(){ console.log('BGM started'); })
              .catch(function(e){ console.log('BGM blocked:', e); alert('Your browser blocked audio. Click Enable Sound again or unmute the tab.'); });
           }
         }
       }
       function playSuccess(){
         var a=document.getElementById('success_sound');
         if(a){
           a.pause();
           a.currentTime=0;
           a.muted=false;
           var p=a.play();
           if(p && typeof p.catch==='function'){ p.catch(function(e){ console.log('Success sound blocked:', e); }); }
         }
       }
       function playWrong(){
         var a=document.getElementById('wrong_sound');
         if(a){
           a.pause();
           a.currentTime=0;
           a.muted=false;
           var p=a.play();
           if(p && typeof p.catch==='function'){ p.catch(function(e){ console.log('Wrong sound blocked:', e); }); }
         }
       }
       Shiny.addCustomMessageHandler('play_unlock', function(x){ playSuccess(); });
    ...
    page_fillable(
      layout_columns(
        ...
        card(
          ...
          HTML(paste(
            "<h4>Structure</h4>",
            "<p>This app is organized as a six-room Shiny escape room, with each chapter built around a different statistics or machine learning concept: randomized block design, threshold tuning and confusion matrix thinking, NLP preprocessing, PCA, random forest, and neural network routing.</p>",
            "<h4>Design and custom theme prompting</h4>",
            "<p>To create the visual style, we used the Shiny Assistant to help us build a custom horror-themed interface. The prompt direction was simple and iterative: we asked it to give us a spooky theme, suggest a font, and then refined it by telling it that the app was an escape room and that we wanted the font to look like dripping text. That guidance led to the dark color palette, eerie visual styling, and the use of a horror-style display font for headers, buttons, and branding.</p>",
            "<h4>What the main innovations are</h4>",
            "<p>Rather than making the instructor search through the code, the main innovations are summarized here directly:</p>",
            "<ul>",
            "<li>A six-room escape-room structure tied to real analytics concepts instead of generic puzzles.</li>",
            "<li>A custom horror-themed Shiny interface with chapter-specific art, atmospheric styling, and escape-room branding.</li>",
            "<li>Interactive puzzle mechanics, including drag-and-drop grouping, threshold input, model selection, and routing decisions.</li>",
            "<li>A floating MERCURY AI helper panel that gives in-world hints and explanations instead of separating the chat from the experience.</li>",
            "<li>Sound design with looping background music plus success and wrong-answer feedback effects.</li>",
            "<li>Chapter-specific success art and alternate ending screens for ESCAPE and PURGE.</li>",
            "</ul>",
            "<h4>MERCURY</h4>",
            "<p>The Romance god of messenger is preserved and moved into a collapsible bottom-right chat panel so it behaves more like an in-world assistant instead of permanently occupying a full column.</p>",
            "<h4>Art and UI assets</h4>",
            "<p>The room art is wired to chapter-specific image assets so the visuals can be refreshed without changing the puzzle logic. This made it possible to improve the look of the app without rewriting the underlying game structure.</p>",
            sep = ""
          ))
        ),
        card(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$enable_sound, {
    state$sound_enabled <- TRUE
    showNotification("Sound enabled.", type = "message")
  })
  ...
  observeEvent(input$execute_escape, {
    ...
    showModal(modalDialog(
      ...
      easyClose = TRUE,
      footer = NULL,
      app_image("escape.png", alt = "Escape ending", style = "width: 100%; border-radius: 18px; display: block; margin-bottom: 12px;"),
      div(state$ending)
    ))
  })
  ...
  observeEvent(input$execute_purge, {
    ...
    showModal(modalDialog(
      ...
      easyClose = TRUE,
      footer = NULL,
      app_image("purge.png", alt = "Purge ending", style = "width: 100%; border-radius: 18px; display: block; margin-bottom: 12px;"),
      div(state$ending)
    ))
  })
