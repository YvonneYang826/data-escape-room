...
build_chat_client <- function(state) {
      ...
      layout_columns(
        ...
        card(
          ...
          HTML(paste(
            ...
            "</ul>",
            "<h4>MERCURY</h4>",
            "<p>The Roman god of messenger is preserved and moved into a collapsible bottom-right chat panel so it behaves more like an in-world assistant instead of permanently occupying a full column.</p>",
...
server <- function(input, output, session) {
  if (get_anthropic_key() == "") {
    showModal(
      modalDialog(
        ...
        easyClose = FALSE,
        footer = NULL,
        p("Set ANTHROPIC_API_KEY in your .Renviron file and restart R."),
        tags$pre("ANTHROPIC_API_KEY=sk-ant-..."),
        p("This version uses Claude via ellmer::chat_anthropic().")
      )
    )
  }
  ...
  client <- reactiveVal(build_chat_client(state))
